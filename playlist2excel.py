...
def get_spotify_client() -> spotipy.Spotify:
    ...
    - SPOTIFY_REDIRECT_URI: Your Spotify app redirect URI (default: http://127.0.0.1:8888/callback)
    ...
    if 'localhost' in redirect_uri:
        print("   Warning: 'localhost' in redirect URI is deprecated. Replacing with '127.0.0.1'.")
    ...
    if not client_id or not client_secret:
        ...
        print("  - SPOTIFY_REDIRECT_URI (optional, default: http://127.0.0.1:8888/callback)")
        sys.exit(1)
    ...
    auth_manager = SpotifyOAuth(
        ...
        scope=scope,
        open_browser=False  # Prevents automatic browser opening
    )
...
def get_user_playlists(sp: spotipy.Spotify) -> List[Dict[str, Any]]:
    ...
    print(f"   Total playlists reported by Spotify API: {total}")
    
    while results:
        batch_size = len(results['items'])
        playlists.extend(results['items'])
        print(f"   Fetched {batch_size} playlists (total: {len(playlists)})")
        if results['next']:
...
def get_playlist_tracks(sp: spotipy.Spotify, playlist_id: str) -> List[Dict[str, Any]]:
    ...
    while results:
        ...
        print(f"   Fetched {batch_size} tracks (total: {len(tracks)})")
...
def get_liked_tracks(sp: spotipy.Spotify) -> List[Dict[str, Any]]:
    """
    Fetch all liked tracks for the authenticated user.
    ...
    while results:
        for item in results['items']:
            ...
            track_info = {
                ...
                'Popularity': track.get('popularity', 0),
                'Added At': item.get('added_at', 'N/A'),
                'Added By': 'Liked',  # Since it's liked
                'Spotify URL': track.get('external_urls', {}).get('spotify', 'N/A')
            }
            tracks.append(track_info)
        
        print(f"   Fetched {len(results['items'])} liked tracks (total: {len(tracks)})")
...
def main():
    ...
    print("   (Copy the entire URL from your browser's address bar after logging in)")
    print()
    ...
    try:
        ...
        print("   Please check your Spotify credentials and redirect URI.")
        sys.exit(1)
    ...
    for idx, playlist in enumerate(playlists, 1):
        ...
        print(f"   [{idx}/{len(playlists)}] Processing '{playlist_name}' ({track_count} tracks)...")
        ...
        if tracks:
            ...
            print(f"   ✓ Added sheet '{sheet_name}' with {len(tracks)} tracks")
        else:
            print(f"   ⚠ No tracks found for '{playlist_name}'")
